...
function systemDesignPrepAnswer(sdCount: number): string {
  return `Start after you are comfortable with core DSA patterns. Read one framework article, then practise one case study per week out loud (URL shortener, rate limiter, news feed, chat). DSA Solutions publishes ${sdCount} in-depth system design articles at ${SITE_URL}/system-design, including payments, Uber, Airbnb, Zoom, Yelp, key-value store, job scheduler, web crawler, Netflix, search engine, ticketing, Redis, e-commerce, Pastebin, leaderboard, API gateway, Google Docs, Instagram, CAP theorem, sharding, notifications, Kafka, and how LeetCode patterns map to real infrastructure.`
}
  ...
  },
  {
    ...
    a: (
      <>
        ...
        <p className="mt-2">
          Our <Link href="/system-design" className="text-violet-600 dark:text-violet-400 hover:underline">System Design section</Link> has {sdCount} in-depth articles — frameworks, case studies (URL shortener, rate limiter, news feed, chat, payments, Uber, Airbnb, Zoom, Yelp, web crawler, Netflix, search engine, ticketing, e-commerce, notifications, file storage, typeahead, Pastebin, leaderboard, Google Docs, Instagram, key-value store, job scheduler), and fundamentals (caching, Redis, CAP theorem, sharding, Kafka, Snowflake IDs, SQL vs NoSQL, load balancing, API gateway).
        </p>
      </>
    ),
  },
