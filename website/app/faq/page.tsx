...
function systemDesignPrepAnswer(sdCount: number): string {
  return `Start after you are comfortable with core DSA patterns. Read one framework article, then practise one case study per week out loud (URL shortener, rate limiter, news feed, chat). DSA Solutions publishes ${sdCount} in-depth system design articles at ${SITE_URL}/system-design, including payments, Uber, Airbnb, Zoom, Yelp, Spotify, Google Maps, Gmail, CDN, Twitch live streaming, S3 object storage, DNS, OAuth authentication, webhooks, service discovery, feature flags, A/B testing, Bloom filters, distributed counters, MapReduce, distributed logging, ad click aggregator, Google Calendar, recommendation systems, stock trading, DoorDash, Stack Overflow, metrics monitoring, distributed locks, online judge, key-value store, job scheduler, web crawler, Netflix, search engine, ticketing, Redis, e-commerce, Pastebin, leaderboard, API gateway, Google Docs, Instagram, CAP theorem, sharding, notifications, Kafka, and how LeetCode patterns map to real infrastructure.`
}
